...
void loadSymmetric_test(Eigen::SparseMatrix<double> &A, std::string PATH)
{
    std::ifstream fin(PATH);
    long int M, N, L;
    while (fin.peek() == '%')
    ...
    for (size_t i = 0; i < L; i++)
    {
        ...
        if (m != n)
        {
            triple.push_back(Eigen::Triplet<double>(m - 1, n - 1, data/2));
            triple.push_back(Eigen::Triplet<double>(n - 1, m - 1, data/2));
        }
        else{
            triple.push_back(Eigen::Triplet<double>(m - 1, n - 1, data));
        }
...
//     // const bool ok = loadMarket(A, path + "/A_2.mat");
//     // REQUIRE(ok);
//     loadSymmetric_test(A,path + "/A_2.mat");
