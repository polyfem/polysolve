...
namespace polysolve::linear
{
    ...
    void cuDSSSolver::free_device_memory()
    {
        if (MatrixA)
        {
            CHECK_CUDSS(cudssMatrixDestroy(MatrixA));
            MatrixA = nullptr;
        }
        if (MatrixX)
        {
            CHECK_CUDSS(cudssMatrixDestroy(MatrixX));
            MatrixX = nullptr;
        }
        if (MatrixB)
        {
            CHECK_CUDSS(cudssMatrixDestroy(MatrixB));
            MatrixB = nullptr;
        }
        if (solverData)
        {
            CHECK_CUDSS(cudssDataDestroy(cudss_handle, solverData));
            solverData = nullptr;
        }
        if (config)
        {
        ...
        if (d_csrRowOffsets)
        {
            CHECK_CUDA(cudaFree(d_csrRowOffsets));
            d_csrRowOffsets = nullptr;
        }
        if (d_csrColIndices)
        {
            CHECK_CUDA(cudaFree(d_csrColIndices));
            d_csrColIndices = nullptr;
        }
        if (d_csrValues)
        {
